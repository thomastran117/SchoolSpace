...
const authSchema_1 = require("../../src/dto/authSchema");
const httpError_1 = require("../../src/error/httpError");
const validateHook_1 = require("../../src/hooks/validateHook");
describe("Auth DTO Validation", () => {
  describe("Zod Schemas", () => {
    it("accepts valid login payload", () => {
      const result = authSchema_1.LoginSchema.safeParse({
        email: "test@test.com",
    ...
    it("rejects short password", () => {
      const result = authSchema_1.SignupSchema.safeParse({
        email: "test@test.com",
        password: "123",
        role: "student",
        captcha: "captcha",
    ...
    it("accepts valid Microsoft token payload", () => {
      const result = authSchema_1.MicrosoftSchema.safeParse({
        id_token: "x".repeat(50),
      });
      expect(result.success).toBe(true);
    });
    it("accepts valid 6-digit verify code", () => {
      const result = authSchema_1.VerifySchema.safeParse({
        email: "test@test.com",
        captcha: "captcha",
        code: "123456",
      });
  ...
  describe("validate() hook", () => {
    it("passes and assigns validated body", async () => {
      ...
      const handler = (0, validateHook_1.validate)(
        authSchema_1.LoginSchema,
        "body"
      );
      await handler(req);
    ...
    it("throws 400 on empty body", async () => {
      ...
      await expect(handler(req)).rejects.toBeInstanceOf(httpError_1.HttpError);
    });
