...
const unauthorizedError_1 = require("../../src/error/unauthorizedError");
...
describe("AuthController", () => {
  ...
  describe("localAuthenticate", () => {
    ...
    it("rethrows HttpError from service", async () => {
      const err = new unauthorizedError_1.UnauthorizedError({
        statusCode: 401,
      ...
      const reply = makeReply();
      await expect(
        controller.localAuthenticate(req, reply)
      ).rejects.toBeInstanceOf(httpError_1.HttpError);
      expect(logger_1.default.error).not.toHaveBeenCalled(); // controller rethrows HttpError directly
    });
  });
  ...
  describe("localVerifyEmail", () => {
    ...
    it("throws if req.body is missing (your middleware should prevent this in prod)", async () => {
      ...
      const reply = makeReply();
      await expect(
        controller.localVerifyEmail(req, reply)
      ).rejects.toBeInstanceOf(httpError_1.HttpError);
    });
  });
