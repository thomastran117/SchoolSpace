...
const internalServerError_1 = require("../../src/error/internalServerError");
const tokenService_1 = require("../../src/service/tokenService");
...
describe("TokenService (new)", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    uuid_1.v4.mockReturnValue("uuid-token");
    tokenService = new tokenService_1.TokenService({ cacheService: mockCache });
  });
  describe("generateTokens", () => {
    it("returns access+refresh tokens and stores refresh in cache with short TTL when remember=false", async () => {
      jsonwebtoken_1.default.sign.mockReturnValue("access.jwt");
    ...
    it("still returns tokens if cache.set fails (non-fatal) and logs warn", async () => {
      ...
      expect(String(logger_1.default.warn.mock.calls[0][0])).toContain(
  ...
  describe("isRefreshTokenValid", () => {
    ...
    it("rethrows non-401 HttpError", async () => {
      const err = new internalServerError_1.InternalServerError({
        statusCode: 500,
        message: "boom",
      });
  ...
  describe("rotateRefreshToken", () => {
    it("deletes old refresh token, returns new access+refresh, and saves new refresh", async () => {
      mockCache.get.mockResolvedValue(TEST_REFRESH_PAYLOAD);
      jsonwebtoken_1.default.sign.mockReturnValue("new-access.jwt");
  ...
  describe("createEmailCode", () => {
    ...
    it("creates a new email code, hashes it, stores cache, returns code", async () => {
      ...
      bcrypt_1.default.hash.mockResolvedValue("hashed-code");
      const res = await tokenService.createEmailCode(
        "test@test.com",
        "hash123",
        "student"
      ...
      expect(bcrypt_1.default.hash).toHaveBeenCalledWith("100000", 10);
      expect(mockCache.set).toHaveBeenCalledWith(
