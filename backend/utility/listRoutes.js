...
function walk(routerOrLayer, prefix, out) {
  const stack = routerOrLayer.stack || (routerOrLayer.handle && routerOrLayer.handle.stack);
...
function normalize(p){ if (!p) return "/"; if (Array.isArray(p)) p=p[0]; return p.startsWith("/")?p:`/${p}`; }
function join(a,b){ if (!a || a === "/") return b; if (!b || b === "/") return a; return `${a}${b}`; }
...
function mountFromLayer(layer) {
  ...
  if (typeof layer.path === "string" && layer.path.length) return normalize(layer.path);
