...
class CodeService {
  ...
  public async generateEnrollmentCode(
    ...
    try {
      ...
      await this.cache.set<CodeRecord>(
        this.codeKey(token),
        { courseId },
        this.CODE_TTL_SEC
      );
  ...
  public async redeemEnrollmentCodeByCode(code: string): Promise<number> {
    try {
      const normCode = this.normalizeCode(code);
      const token = this.tokenize(normCode);

      const attemptsKey = this.attemptsKey(token);
  ...
  private attemptsKey(token: string) {
    return `enroll:attempts:code:${token}`;
  }
