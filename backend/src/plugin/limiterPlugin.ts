...
export default fp<RateLimiterOptions>(async function rateLimiterPlugin(app, options) {
  ...
  function windowFallback(key: string, windowMs: number, maxRequests: number, now: number) {
  ...
  app.addHook("preHandler", async (request) => {
    ...
    const identity = keyGenerator ? keyGenerator(request) : request.rateLimitIdentity;
    ...
    if (policy.mode === "bucket") {
      ...
      try {
        ...
        while (attempts++ < 5) {
          const raw = await cacheService.get<{ tokens: number; lastRefill: number }>(bucketKey);
